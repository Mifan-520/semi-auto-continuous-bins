...
DTU 上云策略已调整：后续每个仓都由本机通过 GPIO32/GPIO25 串口发送自己的 JSON 行，不再设计成“网关统一代发所有仓”。这样每条 JSON 天然带本机仓号，减少仓号错配风险。
...
| DTU 预留 | 每仓本机 Serial1，TX=GPIO32，RX=GPIO25，默认 9600，发送本仓 JSON |
...
- [ ] 每个仓都使用本机 GPIO32/GPIO25 串口输出 JSON 行
- [ ] 每个仓只发送自己的 JSON，不由 1 号网关统一代发
- [ ] JSON 必须包含本机仓号，避免平台侧混淆仓号
...
- `GPIO32/GPIO25` 是用户指定的 DTU 串口脚，不要换；后续每个仓都用这组串口发送本仓 JSON。
